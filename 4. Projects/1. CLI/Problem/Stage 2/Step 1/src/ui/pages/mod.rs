...
impl Page for HomePage {
    fn draw_page(&self) -> Result<()> {
        ...
        let epics = db_state.epics.iter().sorted_by_key(|x| x.0);
        for epic in epics {
            let id_string = get_column_string(epic.0.to_string().as_str(), 12);
            let name_string = get_column_string(epic.1.name.as_str(), 34);
            let status_string = get_column_string(epic.1.status.to_string().as_str(), 18);
            println!("{}|{}|{}", id_string, name_string, status_string);
        }
    ...
    fn handle_input(&self, input: &str) -> Result<Option<Action>> {
        match input {
            ...
            x => match x.parse::<u32>() {
                Ok(number) => {
                    let binding = self.db.read_db()?;
                    if binding.epics.contains_key(&number) {
                        Ok(Some(Action::NavigateToEpicDetail { epic_id: number }))
                    } else {
                        Ok(None)
                    }
                }
...
impl Page for EpicDetail {
    fn draw_page(&self) -> Result<()> {
        ...
        let id_string = get_column_string(self.epic_id.to_string().as_str(), 6);
        let name_string = get_column_string(epic.name.as_str(), 14);
        let description_string = get_column_string(epic.description.to_string().as_str(), 29);
        let status_string = get_column_string(epic.status.to_string().as_str(), 14);
        println!(
            "{}|{}|{}|{}",
            id_string, name_string, description_string, status_string
        );
        ...
        let stories = epic
            .stories
            .iter()
            .filter_map(|id| db_state.stories.get_key_value(id));
        for story in stories {
            let id_string = get_column_string(story.0.to_string().as_str(), 12);
            let name_string = get_column_string(story.1.name.as_str(), 34);
            let status_string = get_column_string(story.1.status.to_string().as_str(), 18);
            println!("{}|{}|{}", id_string, name_string, status_string);
        }
    ...
    fn handle_input(&self, input: &str) -> Result<Option<Action>> {
        match input {
            ...
            x => match x.parse::<u32>() {
                Ok(number) => {
                    let binding = self.db.read_db()?;
                    if binding.stories.contains_key(&number) {
...
impl Page for StoryDetail {
    fn draw_page(&self) -> Result<()> {
        ...
        let id_string = get_column_string(self.story_id.to_string().as_str(), 6);
        let name_string = get_column_string(story.name.as_str(), 14);
        let description_string = get_column_string(story.description.to_string().as_str(), 29);
        let status_string = get_column_string(story.status.to_string().as_str(), 14);
        println!(
            "{}|{}|{}|{}",
            id_string, name_string, description_string, status_string
        );
